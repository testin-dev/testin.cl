...
(function() {
    ...
    if ($("#cardRadioone , #cardRadioTwo").length && ($("#internetpayment").hide(),
    $("#cardRadioone").on("change", (function() {
        this.checked && ($("#cardpayment").show(),
        $("#internetpayment").hide())
    }
    )),
    $("#cardRadioTwo").on("change", (function() {
        this.checked && ($("#internetpayment").show(),
        $("#cardpayment").hide())
    }
    ))),
    ...
    if ($("input").length && Inputmask().mask(document.querySelectorAll("input")),
    $("#editor").length)
        new Quill("#editor",{
            modules: {
                toolbar: [[{
    ...
    $("#liveAlertPlaceholder").length) {
        ...
        a && a.addEventListener("click", (function() {
            ...
            t = "success",
            (a = document.createElement("div")).innerHTML = '<div class="alert alert-' + t + ' alert-dismissible" role="alert">' + e + '<button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close"></button></div>',
            o.append(a)
        }
        ))
    }
...
dragula([document.querySelector("#do"), document.querySelector("#progress"), document.querySelector("#review"), document.querySelector("#done")]),
$("#courseForm").length) && document.addEventListener("DOMContentLoaded", (function() {
    ...
    e.fn.downCount = function(t, o) {
        ...
        var r = this
          ...
          , n = setInterval((function() {
            ...
            r.find(".days").text(d),
            r.find(".hours").text(h),
            r.find(".minutes").text(c),
            r.find(".seconds").text(p),
            r.find(".days_ref").text(m),
            r.find(".hours_ref").text(u),
            r.find(".minutes_ref").text(f),
            r.find(".seconds_ref").text(g)
        }
        ), 1e3)
    }
}(jQuery),
